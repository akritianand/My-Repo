<?php
        ...
        if ($result->num_rows > 0) {
            while($row = $result->fetch_assoc()){
                if ($row['ticket_open_close'] == 'Open') {
                    ...
                    $sql2 = "SELECT distinct Ticket.ticket_id, Emp1.emp_id as e1, Emp2.emp_id as e2, Emp1.emp_firstname as openfname, Emp1.emp_lastname as openlname, Emp2.emp_firstname as closefname, Emp2.emp_lastname as closelname, System.system_name, Equipments.equip_name, Ticket.ticket_open_empid, Ticket.ticket_close_empid, Ticket.ticket_open_close, Ticket.ticket_open_datetime, Ticket.ticket_close_datetime, Station.station_name, Ticket.ticket_duration FROM Ticket, Station, System, Equipments, Employee as Emp1, Employee as Emp2 WHERE 
    ...
    if (isset($_POST['mainstaff']) && $_POST['mainstaff']) {
        $sql1 = "SELECT Ticket.ticket_open_close FROM Ticket, Employee AS Emp2 WHERE Ticket.ticket_close_empid = $mainstaff AND (Ticket.ticket_open_close = 'Close' OR Ticket.ticket_open_close = 'Pending')";
        $result = $link->query($sql1);
        if ($result->num_rows > 0) {
            while($row = $result->fetch_assoc()){
                if ($row['ticket_open_close'] == 'Open') {
                    ...
                    $sql2 = "SELECT distinct Ticket.ticket_id, Emp1.emp_id as e1, Emp2.emp_id as e2, Emp1.emp_firstname as openfname, Emp1.emp_lastname as openlname, Emp2.emp_firstname as closefname, Emp2.emp_lastname as closelname, System.system_name, Equipments.equip_name, Ticket.ticket_open_empid, Ticket.ticket_close_empid, Ticket.ticket_open_close, Ticket.ticket_open_datetime, Ticket.ticket_close_datetime, Station.station_name, Time(Ticket.ticket_duration) AS duration FROM Ticket, Station, System, Equipments, Employee as Emp1, Employee as Emp2 WHERE Ticket.ticket_close_empid = $mainstaff AND (Ticket.ticket_open_close = 'Close' OR Ticket.ticket_open_close = 'Pending') AND Station.station_id = Ticket.ticket_station_id AND Ticket.ticket_system_id = System.system_id AND Equipments.equip_id = Ticket.ticket_equip_id AND Ticket.ticket_open_empid = Emp1.emp_id AND Ticket.ticket_close_empid = Emp2.emp_id AND Ticket.ticket_close_empid = $mainstaff" ;
                }
            }
        }
    }
    ...
    echo "<thead><tr><th style='width: 200px;'>Ticket ID</th><th>System</th><th>Equipment</th><th>Opened at</th><th>Station</th><th>Status</th><th>Closed by</th><th>Closed at</th><th>Duration</th></tr></thead>";

    if ($result->num_rows > 0) {
        while($row = $result->fetch_assoc()){
            $linkaddress = "cssjobdetailsubmit.php?ticketid=".$row['ticket_id'];

            echo "<tr><td style='width: 200px;'><a target='_blank' href='".$linkaddress."'>".$row['ticket_id']."</a></td><td>".$row['system_name']."</td><td>".$row['equip_name']."</td><td>".$row['station_name']."</td><td>".$row['ticket_open_close']."</td><td>".$row['closefname']." ".$row['closelname']." (".$row['e2'].")</td><td>".$row['ticket_close_datetime']."</td><td>".$row['duration']."</td></tr>";
    ...
    if ($result->num_rows > 0) {
        while($row = $result->fetch_assoc()){
            ...
            echo "<tr><td style='width: 200px;'><a target='_blank' href='".$linkaddress."'>".$row['ticket_id']."</a></td><td>".$row['system_name']."</td><td>".$row['equip_name']."</td><td>".$row['ticket_open_datetime']."</td><td>".$row['station_name']."</td><td>".$row['ticket_open_close']."</td><td>".$row['closefname']." ".$row['closelname']." (".$row['e2'].")</td><td>".$row['ticket_close_datetime']."</td><td>".$row['duration']."</td></tr>";
